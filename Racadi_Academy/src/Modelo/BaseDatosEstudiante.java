...
public class BaseDatosEstudiante {
    ...
    public Boolean comprobarLogin(String tryCorreo , String tryContraseña ){
        Estudiante datosEsstudiante = null;

        for ( Estudiante estudiante: BaseDatos.values() ){
            if (estudiante.getCorreo().equals(tryCorreo)){
                datosEsstudiante=estudiante;
            }
        }

        if (datosEsstudiante!=null) {
            if (datosEsstudiante.getContraseña().equals(tryContraseña)){
                vistaEstudiante.mostrarMensaje("Bienvenido "+datosEsstudiante.getNombre() );
                return true;
            }else {
                vistaEstudiante.mostrarMensaje("contraseña invalida" );
                return false;
            }
        }else {
            vistaEstudiante.mostrarMensaje("correo invalido" );
            return false;
        }
    }
